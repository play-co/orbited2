...
B(j,n);l[h]=j.exports}}n=l[h];if(g==1)f=n;if(e.as){h=e.as.match(/^\.*(.*?)\.*$/)[1];e=h.split(".");l=e.length-1;j=b;for(var p=0;p<l;++p){var w=e[p];if(w){j[w]||(j[w]={});j=j[w]}}j[e[l]]=n;if(g>1)f[h]=n}else if(e["import"])if(e["import"]["*"])for(p in l[h])b[p]=n[p];else try{for(p in e["import"])b[e["import"][p]]=n[p]}catch(M){i.log("module: ",l);throw M;}}return f}var i,y={"net.env":{src:'function getObj(c,a,b){try{jsio("from .env."+(b||jsio.__env.name)+"."+a+" import "+c+" as result")}catch(d){throw logger.error("Invalid transport (",a,") or environment (",b,")");}return result}exports.getListener=bind(this,getObj,"Listener");exports.getConnector=bind(this,getObj,"Connector");\n',
filePath:"jsio/net/env.js"},Orbited2:{src:'jsio("from net.protocols.buffered import BufferedProtocol");jsio("import net");jsio("import std.uri");jsio("import std.JSON");jsio("import std.utf8 as utf8");exports.logging=logging;exports.logger=logger;var originalWebSocket=window.WebSocket,baseUri=new std.uri.Uri(window.location),defaultOrbitedUri;\nfunction setup(){for(var b=document.getElementsByTagName("script"),c=0,d;d=b[c];++c)if(d.src.match("(^|/)Orbited.js$")){found=true;b=new std.uri.Uri(d.src.substring(0,d.src.length-10));defaultOrbitedUri=(b.getProtocol()||baseUri.getProtocol())+"://"+(b.getHost()||baseUri.getHost())+":"+(b.getPort()||baseUri.getPort()||"80")+(b.getPath()||baseUri.getPath());break}}setup();var READYSTATE_WAITING=-1,READYSTATE_CONNECTING=0,READYSTATE_OPEN=1,READYSTATE_CLOSING=2,READYSTATE_CLOSED=3;\nexports.TCPSocket=Class(function(){this.init=function(a){this._orbitedUri=a.orbitedUri||defaultOrbitedUri;this._orbitedUri.match("/$")||(this._orbitedUri+="/");this.readyState=READYSTATE_WAITING;this._buffer="";this._forceTransport=a.forceTransport};this.open=function(a,f,g){if(!this._orbitedUri)throw Error("Could not automatically determine Orbited\'s uri, and Orbited.TCPSocket.opts.orbitedUri was not manually specified in TCPSocket constructor");var h=getMultiplexer(this._orbitedUri,this._forceTransport);\nthis._isBinary=!!g;this._host=a;this._port=f;this._conn=h.openConnection();this._conn.onOpen=bind(this,c);this._conn.onFrame=bind(this,d);this._conn.onClose=bind(this,b);this.readyState=READYSTATE_CONNECTING};var b=function(){if(this.readyState!=READYSTATE_CLOSED){this.readyState=READYSTATE_CLOSED;releaseMultiplexer();bind(this,e)("close")}},c=function(){this._conn.send(JSON.stringify({hostname:this._host,port:this._port,protocol:"tcp"}))},d=function(a){switch(this.readyState){case READYSTATE_CONNECTING:if(a==\n"1"){this.readyState=READYSTATE_OPEN;bind(this,e)("open")}else this._onClose();break;case READYSTATE_OPEN:this._buffer+=a;var f=utf8.decode(this._buffer);a=f[0];this._buffer=this._buffer.slice(f[1]);a.length&&bind(this,e)("read",a);break}};this.send=function(a){if(!(this.readyState>=READYSTATE_CLOSING)){this._isBinary||(a=utf8.encode(a));this._conn.send(a)}};this.close=function(){if(!(this.readyState>=READYSTATE_CLOSING)){this.readyState=READYSTATE_CLOSING;this._conn.close();this._conn.onClose=bind(this,\n"_onClose")}};var e=function(a,f){var g=this["on"+a];g&&g.call(this,f)}});\nexports.WebSocket=Class(function(){this.init=function(a){this.URL=a;this.readyState=READYSTATE_CONNECTING;this.bufferedAmount=0;this._conn=getMultiplexer(exports.WebSocket.opts.orbitedUri).openConnection();this._conn.onOpen=bind(this,c);this._conn.onFrame=bind(this,d);this._conn.onClose=bind(this,b);this.onopen=this.onmessage=this.onerror=this.onclose=null};this.send=function(a){if(!(this.readyState>=READYSTATE_CLOSING)){a=utf8.encode(a);this._conn.send(a)}};var b=function(){if(this.readyState!=READYSTATE_CLOSED){this.readyState=\nREADYSTATE_CLOSED;releaseMultiplexer();bind(this,e)("close")}},c=function(){var a=new std.uri.Uri(this.URL);this._conn.send(JSON.stringify({hostname:a.getHost(),port:parseInt(a.getPort())||(a.getProtocol()=="ws"?80:443),path:a.getPath()||"/",protocol:"ws_"+exports.WebSocket.opts.protocolVersion}))},d=function(a){switch(this.readyState){case READYSTATE_CONNECTING:if(a=="1"){this.readyState=READYSTATE_OPEN;bind(this,e)("open")}else this._onClose();break;case READYSTATE_OPEN:a=utf8.decode(a);bind(this,\ne)("message",a[0]);break}},e=function(a,f){var g=this["on"+a];g&&g.call(this,{data:f})};this.close=function(){if(!(this.readyState>=READYSTATE_CLOSING)){this.readyState=READYSTATE_CLOSING;this._conn.close();this._conn.onClose=bind(this,b)}}});var installed=false;\nexports.WebSocket.install=function(b){if(installed)throw Error("orbited.Websocket already installed");validateOpts(b);exports.WebSocket.opts=b;installed=true;if(!(!b.forceProxy&&hasNativeWebSocket(b.protocolVersion))){window.WebSocket=exports.WebSocket;window.WebSocket.__original__=originalWebSocket}};\nfunction validateOpts(b){b.protocolVersion=b.protocolVersion||"hixie76";b.forceProxy=!!b.forceProxy;if(!b.orbitedUri)b.orbitedUri=defaultOrbitedUri;if(!b.orbitedUri)throw Error("orbitedUri not specified; unable to auto-detect it based on script tag includes");b.orbitedUri.match("/$")||(b.orbitedUri+="/")}\nfunction hasNativeWebSocket(b){if(!originalWebSocket)return false;if(!b)return true;if(navigator.userAgent.match("Chrome/5"))return b=="hixie75";if(navigator.userAgent.match("Chrome/6"))return b=="hixie76";if(navigator.userAgent.match("Version/5.0 Safari"))return b=="hixie75";return false}var multiplexer=null,count=0;\nfunction getMultiplexer(b,c){logger.debug("getMultiplexer",b,c);if(!multiplexer){multiplexer=new OrbitedMultiplexingProtocol(b);multiplexer.onClose=function(){multiplexer=null};var d=c;d||(d=originalWebSocket?"ws":"csp");logger.debug("_transport is",d);switch(d){case "ws":d=new std.uri.Uri(b);d.setProtocol("ws");d=d.render()+"ws";logger.debug("connecting with ws");net.connect(multiplexer,"websocket",{url:d,wsConstructor:originalWebSocket});multiplexer.connectionLost=bind(multiplexer,"_connectionLost",\n"websocket");multiplexer.mode="ws";break;case "csp":logger.debug("connecting with csp");net.connect(multiplexer,"csp",{url:b+"csp"});multiplexer.mode="csp";multiplexer.connectionLost=bind(multiplexer,"_connectionLost","csp");break}}count+=1;return multiplexer}function releaseMultiplexer(){if(--count==0){multiplexer.transport.loseConnection();multiplexer=null}}var OPEN_FRAME=0,CLOSE_FRAME=1,DATA_FRAME=2;DELIMITER=",";\nvar Connection=Class(function(){this.init=function(b,c){this._multiplexer=b;this._id=c};this.send=function(b){this._id=id;this._multiplexer.sendFrame(this._id,DATA_FRAME,b)};this.close=function(){this._multiplexer.closeConnection(this._id)};this.onFrame=function(){};this.onClose=function(){};this.onOpen=function(){}}),OrbitedMultiplexingProtocol=Class(BufferedProtocol,function(b){this.init=function(c){b(this,"init",arguments);this._baseUri=c;this._connections={};this._size=this._last_id=-1;this._connected=\nfalse};this.openConnection=function(){logger.debug("opening multiplexing connection");var c=++this._last_id,d=this._connections[c]=new Connection(this,c);if(this._connected){logger.debug("already opened, triggering now");this._sendOpen(c)}return d};this.closeConnection=function(c){this._connections[c]&&this._connected&&this._sendClose(c)};this._sendOpen=function(c){this.sendFrame(c,OPEN_FRAME)};this._sendClose=function(c){this.sendFrame(c,CLOSE_FRAME)};this.sendFrame=function(c,d,e){e=e||"";if(!this._connected)throw Error("Multiplexer not connected");\nc=c+DELIMITER+d+DELIMITER+e;this.transport.write(c.length+DELIMITER+c)};this.connectionMade=function(){logger.debug("connectionMade on multiplexer");this._connected=true;for(id in this._connections)this._sendOpen(id)};this.onClose=function(){};this._connectionLost=function(c,d,e){if(e)this.onClose();else if(c=="websocket"){net.connect(this,"csp",{url:this._baseUri+"csp"});this.connectionLost=bind(this,"_connectionLost","csp");this.mode="csp"}};this._dispatchPayload=function(c){var d=c.indexOf(DELIMITER);\nif(d!=-1){var e=parseInt(c.substring(0,d)),a=c.indexOf(DELIMITER,d+1);if(a!=-1){d=parseInt(c.substring(d+1,a));c=c.substring(a+1);a=this._connections[e];if(!(!a||typeof d!="number"))switch(d){case OPEN_FRAME:a.onOpen();break;case CLOSE_FRAME:delete this._connections[e];a.onClose();break;case DATA_FRAME:a.onFrame(c);break}}}};this.bufferUpdated=function(){for(;;){if(this._size==-1)if(this.buffer.hasDelimiter(DELIMITER)){this._size=parseInt(this.buffer.consumeToDelimiter(DELIMITER));this.buffer.consumeBytes(DELIMITER.length)}else break;\nif(!this.buffer.hasBytes(this._size))break;this._dispatchPayload(this.buffer.consumeBytes(this._size));this._size=-1}}});\n',
